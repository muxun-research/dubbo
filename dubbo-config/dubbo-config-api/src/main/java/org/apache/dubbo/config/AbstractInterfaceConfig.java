...
public abstract class AbstractInterfaceConfig extends AbstractMethodConfig {
	...
	/**
	 * 构建公共属性参数部分
	 * 包括dubbo版本号，使用dubbo包版本号，PID，时间戳
	 * @param map 属性集合
	 */
	...
	/**
	 * 加载注册信息，并且把它转换为URL
	 * 优先级是：system property -> dubbo registry config
	 * @param provider 身份是否是provider
	 * @return 注册url集合
	 */
	protected List<URL> loadRegistries(boolean provider) {
		// check && override if necessary
		// 在有必要的情况下，检查并重写
		// 使用nacos作为注册中心时，一个示例：<dubbo:registry address="localhost" protocol="spring-cloud" valid="true" zookeeperProtocol="false" prefix="dubbo.registries." id="spring-cloud" />
		...
		if (CollectionUtils.isNotEmpty(registries)) {
			for (RegistryConfig config : registries) {
				...
				if (!RegistryConfig.NO_AVAILABLE.equalsIgnoreCase(address)) {
					...
					// 添加应用信息
					// 此处给出一个application示例：<dubbo:application name="dubbo-client-second" valid="true" qosEnable="false" qosEnableCompatible="false" prefix="dubbo.application" id="dubbo-client-second" />
					appendParameters(map, application);
					// 添加配置信息
					// 此处给出一个config示例：<dubbo:registry address="localhost" protocol="spring-cloud" valid="true" zookeeperProtocol="false" prefix="dubbo.registries." id="spring-cloud" />
					appendParameters(map, config);
					// 添加RegistryService
					map.put(PATH_KEY, RegistryService.class.getName());
					// 添加运行时信息
					appendRuntimeParameters(map);
					// 如果没有设置协议类型，则默认使用dubbo协议
					...
					for (URL url : urls) {
						url = URLBuilder.from(url)
								...
								// 设置协议为registry协议，为注册协议
								.setProtocol(REGISTRY_PROTOCOL)
								// 重新构建注册协议请求
								// 示例：registry://localhost:9090/org.apache.dubbo.registry.RegistryService?application=dubbo-client-second&dubbo=2.0.2&pid=3363&qos.enable=false&registry=spring-cloud&release=2.7.3&timestamp=1568864352047
								.build();
						// 在身份为provider，并且有register属性
						// 或者身份不是provider，但是有subscribe属性时
						// 添加注册请求url
	 ...
	 * 通过system properties加载MonitorConfig，并将其转换为URL
	 * @param registryURL 监控信息注册的URL
	 * @return 增加了监控信息的URL
	 */
	protected URL loadMonitor(URL registryURL) {
		checkMonitor();
		Map<String, String> map = new HashMap<String, String>();
		// 接下来和loadRegistries行为类似
		// monitor的接口
